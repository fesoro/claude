...
### Tapşırıq 1: In-Context Memory Management

`MemoryManager` sinifi yaz: `messages` massivinin token sayı 150,000-i keçdikdə ən köhnə non-system mesajları sil. Lakin həmişə: ilk 3 mesajı (context establishment) saxla. Bir həftəlik söhbət logu üzərindən test et — context quality düşürmü?

### Tapşırıq 2: Episodic Memory Store

`agent_memories` cədvəli yarat. Agent tapşırığı bitirdikdə mühüm factları (user preference, key decision, past action) embed edib bu cədvəldə saxla. Növbəti session-da `SIMILARITY > 0.8` filter ilə ən relevant memory-ləri çəkib system prompt-a daxil et. 5 söhbət sonra memory-nin cavab keyfiyyətinə təsirini ölç.

### Tapşırıq 3: Memory Privacy Audit

Agent-in `agent_memories` cədvəlindəki saxladığı məlumatları nəzərdən keçir. PII (ad, email, telefon) var mı? Şifrə, API key kimi həssas data var mı? `MemoryFilter` yaz: bu tip məlumatları saxlamadan əvvəl redact et. GDPR "silinmə hüququ" tələbini nəzərə alaraq `DELETE FROM agent_memories WHERE user_id = ?` support et.

---

## Əlaqəli Mövzular

- `05-build-custom-agent-laravel.md` — Memory sisteminin agent loop-a inteqrasiyası
- `09-human-in-the-loop.md` — Paused agent session memory-si
- `13-agent-security.md` — Memory-də saxlanan həssas data qorunması
- `../04-rag-embeddings/01-embeddings-vector-search.md` — Semantic memory üçün embedding

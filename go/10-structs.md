...
Go-dakı `struct` — müxtəlif tipli məlumatları bir araya gətirən tip sistemidir. PHP-dəki class-a bənzər, lakin inheritance (varislik) yoxdur. Struct-lar Go-da OOP-nin əsasını təşkil edir: methodlar struct-lara bağlanır, embedding ilə komponent-bazalı kod yazmaq mümkündür. Go-nun bir fəlsəfəsi var: composition over inheritance.

## Niyə Vacibdir

PHP developer üçün struct — class-ın lightweight variantıdır. Verilənlər bazası model-ləri, API request/response tipləri, domain entity-lər — hamısı struct ilə modelləşdirilir. JSON serialization üçün struct tag-ları (`json:"..."`) isə Laravel-dəki resource/transformer-ın Go ekvivalentidir. Constructor function pattern-i PHP-dəki `__construct`-ın yerinə keçir.
...
- **Constructor function** — `func NewUser(...) User` — PHP-dəki `new User(...)` ekvivalenti
...
- PHP inheritance (`extends`) yoxdur; Go-da embedding (composition) var
...
// Constructor — PHP-dəki __construct ekvivalenti

...
1. Laravel-də structured JSON logging qurun: Monolog `JsonFormatter`, hər log entry-ə `request_id`, `user_id`, `environment`, `service` field-ları əlavə edin; `tap` ilə custom processor; `grep '"level":"ERROR"' storage/logs/laravel.log | jq .` ilə filtr edin
2. Telescope-u development, Pulse-u production üçün konfigurasiya edin: Telescope `isRunningInConsole()` yoxlaması, Pulse `pruneable` job-u schedule edin; hər ikisi üçün dashboard qurun; production-da sensitive məlumatların loglanmadığını yoxlayın
3. Alert fatigue-i azaldın: mövcud alert-ləri audit edin — son 30 gündə neçəsi actionable oldu; duplicate alert-ləri `inhibit_rules` ilə suppress edin; `group_by` ilə eyni problem üçün çoxsaylı alert-ləri birləşdirin; alert-ləri priority ilə etiketləyin
4. Log aggregation pipeline qurun: Laravel → Filebeat → Logstash (grok parse + enrich) → Elasticsearch; `curl -X POST` ilə test log göndərin; Kibana-da görün; stack trace multiline düzgün parse edilirmi yoxlayın
5. Liveness vs Readiness probe fərqini praktiki test edin: `readinessProbe` DB connection yoxlasın — DB-ni dayandırın, pod-un service-dən çıxarıldığını (`READY 0/1`) görün; `livenessProbe` sadə HTTP 200 qaytarsın — pod restart olmasın; hər iki probe-u `kubectl describe pod`-da oxuyun
6. SLA dashboard yaradın: Grafana-da 99.9% availability hədəfini göstərən panel; `1 - (error_count / total_count)` hesablama; son 30 günün actual availability-si; error budget qalan dəqiqələri; burn rate indicator

## Əlaqəli Mövzular

- [Prometheus](18-monitoring-prometheus.md) — metrics, PromQL, alerting
- [Grafana](19-monitoring-grafana.md) — dashboard, alert visualization
- [ELK Stack](20-elk-stack.md) — log aggregation, Kibana
- [Observability](42-observability.md) — 3 pillar (metrics/logs/traces)
- [SLA/SLO/SLI](43-sla-slo-sli.md) — error budget, SLO tracking

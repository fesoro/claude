...
**1. Bütün technical debt-i "pis" saymaq**
Deliberate/Prudent debt — MVP-ni vaxtında çatdırmaq üçün bilinə-bilinə götürülən short-cut — tamamilə məqbuldur, hətta zəruridir. "Heç debt olmamalıdır" deyib hər şeyi mükəmməl etməyə çalışmaq: market window itir, rəqib çatdırır, product ölür. Fərqi bil: *inadvertent* debt (bilmədən yaranır, pis) vs *deliberate/prudent* debt (bilinə-bilinə, vaxt qazanmaq üçün, sonradan plan var).

```
Deliberate/Prudent TD nümunəsi:
  "Launch üçün caching layer yoxdur — DB-yə birbaşa gedirik.
   Launch-dan sonrakı Q1-də Redis əlavə edəcəyik.
   JIRA: TECH-234"
  → Bu intentional debt-dir. Yanlış deyil.

Deliberate/Reckless TD nümunəsi:
  "Test yazmayacağıq, vaxt yoxdur"
  → Nəticəsini bilmirik, sonrakı plan yoxdur. Bu zərərlidir.
```

**2. Technical debt-i track etmədən biriktirmək**
Debt götürüb heç yerdə qeyd etməmək — "sonra edərik" demək amma heç vaxt etməmək. 6 aydan sonra niyə bu kompromisi seçdiyini heç kim xatırlamır, yeni developer həmin "temporary" kodu permanent sanır. Hər intentional debt bir ticket olmalıdır: nə, niyə, nə vaxt düzəldəcəksiniz.

```php
// YANLIŞ — sadəcə TODO
// TODO: bu cache əlavə et
$users = User::all(); // N+1 problem

// DOĞRU — tracked debt
// TECH-456: Redis cache əlavə et (Q2 2026)
// Şərh: Launch rush-unda buraxıldı, benchmark: ~200ms/req overhead var
$users = User::all();
```
...
- [ADR](06-architecture-decision-records.md) — qərarları izləmək debt-in gözlənilmədən yaranmasını önləyir
- [Code Smells & Refactoring](02-code-smells-refactoring.md) — debt-in görünən əlamətləri
- [Hexagonal Architecture](../architecture/05-hexagonal-architecture.md) — debt-i izolə edən arxitektura
- [Modular Monolith](../architecture/08-modular-monolith.md) — debt-in modul sərhədlərindən kənara çıxmasını önləyir

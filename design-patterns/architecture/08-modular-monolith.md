...
**Sıx coupled modules** — bir modulu dəyişmək başqasını qırır. Əlamətlər: Billing modulu Payment modelini `use App\Modules\Payment\Internal\Transaction` ilə birbaşa import edir; modullar arası cross-module JOIN-lar var; shared Eloquent model hər modul tərəfindən import olunur. Nəticə: module-ları ayrı microservice-ə çevirmək mümkünsüzləşir, "modular monolith" sözü ilə adi monolit arasında fərq qalmır. Hər modul yalnız digərinin public API-si, interface-i ya da event-i ilə danışmalıdır.

**Architecture erosion** — vaxt keçdikcə "tez həll" üçün sərhədlər pozulur. Bu yavaş-yavaş baş verən ən qorxulu anti-pattern-dir. `phpat` ilə CI/CD-də architecture test-lər yazılmadan yalnız code review-a güvənmək bu pozulmaları qaçırır.

...
Jeffrey Palermo tərəfindən 2008-ci ildə təqdim edildi. Məqsəd: **domain logic-i kənar dünyadan tamamilə izolə etmək**. Ən iç qat Domain Model (entity-lər, value object-lər, domain event-lər) — heç bir xarici asılılıq yoxdur. Kənardan içəriyə: Infrastructure → Application Services → Domain Services → Domain Model. Asılılıq həmişə içəriyə baxır.

## Niyə Vacibdir

Tipik Laravel layihəsinin böyük problemi: `User extends Model` — domain entity-si Eloquent-ə bağlıdır, framework olmadan test etmək mümkünsüzdür. DB sxeması dəyişdikdə domain pozulur. Onion Architecture bu problemi fundamental həll edir: domain pure PHP-dir, Infrastructure-da Mapper pattern domain ↔ Eloquent model çevrilməsini edir. Framework-i dəyişmək mümkün olur, test-lər ms-lər içindədir.

## Əsas Anlayışlar

- **Domain Model** (ən iç): Entity, Value Object, Domain Event, Aggregate — xarici asılılıq yoxdur
- **Domain Services**: bir neçə entity/aggregate ilə işləyən domain məntiqi; infrastructure asılılığı yoxdur
- **Application Services**: use case-lər; domain service-ləri, repository-ləri orchestrate edir; transaction boundary-lər burada
- **Infrastructure**: DB, Email, Message Queue — konkret implementasiyalar; domain interface-lərini implement edir
- **Dependency Rule**: daha kənardakı qat içəridəkindən asılı ola bilər; əksi QADAĞANDIR
- **Repository interface**: Domain layer-da tərif edilir; Infrastructure-da implement olunur — bu DIP-in tətbiqidir
- **Mapper pattern**: Eloquent model ↔ Domain entity çevrilməsi; Infrastructure-da yaşayır; domain-i DB sxemasından izolə edir

## Praktik Baxış

- **Real istifadə**: mürəkkəb domain qaydaları olan enterprise tətbiqlər; DDD ilə birlikdə; uzun ömürlü, böyük komanda; framework migration planı olan layihələr
- **Trade-off-lar**: framework-dən müstəqil domain, yüksək testability, DB dəyişimi asanlaşır; lakin əhəmiyyətli boilerplate (Entity + Mapper + Repository interface + Adapter + Model), yüksək ilkin setup vaxtı
- **Hansı hallarda istifadə etməmək**: sadə CRUD tətbiqləri; startup MVP; domain logic minimaldırsa — adi Laravel structure daha praktikdir
- **Common mistakes**: Domain entity-lərdə Eloquent-i extend etmək; Application Service-ə domain logic yazmaq; Infrastructure-dan domain-ə birbaşa çağırış

### Anti-Pattern Nə Zaman Olur?

**Anemic model sarılmış layer-lər**: Domain entity-lər yalnız getter/setter-li data class-lardır — `$order->setStatus('confirmed')` — heç bir domain rule yoxdur. Onion Architecture qurulub amma bütün business logic Application Service-dədir. Bu "Onion var, amma domain logic yoxdur" xəstəliyidir. Entity-lər davranış daşımalıdır: `$order->confirm()`, `$order->cancel()`, `$order->addItem()`.
...
1. Mövcud `Order extends Model` entity-sini Onion-a köçürün: `App\Domain\Order\Entity\Order` (pure PHP), `App\Infrastructure\Persistence\Order\OrderModel extends Model`, `App\Infrastructure\Persistence\Order\OrderMapper` (toDomain/toModel)
2. `Money` value object yaradın: `of(int $amount, string $currency)`, `add()`, `multiply()`, `equals()`; Unit test yazın — Laravel-siz, ms-lər içindədir
3. `PlaceOrderService` application service yazın: `OrderRepository` (interface), `ProductRepository` (interface), `EventDispatcher` (interface) inject edin; unit test-lərdə mock ilə test edin
4. `deptrac` konfiqurasiya edin: Domain layer Infrastructure-a asılılığını qadağan edin; mövcud layihənizin neçə pozuntusu var — tapın və düzəldin

## Əlaqəli Mövzular

- [Hexagonal Architecture](05-hexagonal-architecture.md) — Ports & Adapters — Onion ilə oxşar prinsiplər, fərqli terminologiya
- [Layered Architectures](04-layered-architectures.md) — müqayisəli baxış
- [DDD](../ddd/01-ddd.md) — Onion DDD ilə ən yaxşı uyğunlaşır; Entity, VO, Domain Event burada
- [Value Objects](../ddd/02-value-objects.md) — Domain Model-in əsas building block-larından biri
- [Aggregates](../ddd/04-ddd-aggregates.md) — Domain Model qatında yaşayır
- [Repository Pattern](../laravel/01-repository-pattern.md) — Domain layer-da interface; Infrastructure-da implementasiya
- [CQRS](../integration/01-cqrs.md) — Application Services CQRS ilə tamamlanır

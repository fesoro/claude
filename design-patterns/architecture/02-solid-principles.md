...
SOLID — object-oriented proqramlaşdırmanın 5 əsas prinsipinin baş hərflərindən ibarət akronimdir. Robert C. Martin (Uncle Bob) tərəfindən formalaşdırılmışdır. Məqsəd: daha davamlı, genişlənə bilən və test edilə bilən kod yazmaq.

## Niyə Vacibdir

5+ illik PHP developer kimi SOLID-i bilmək kifayət deyil — onu real layihələrdə tətbiq etməyi bacarmalısınız. Controller-lər şişirsə, test yazmaq çətinləşirsə, bir yerə toxunduqda başqa yer sınırsa — adətən SOLID pozulur. Bunları müəyyən etmək və düzəltmək Senior developer-in əsas bacarığıdır.

## Əsas Anlayışlar

- **SRP** (Single Responsibility): bir class-ın yalnız bir dəyişmə səbəbi olmalıdır
- **OCP** (Open/Closed): genişlənməyə açıq, dəyişdirilməyə bağlı
- **LSP** (Liskov Substitution): alt-class-lar üst-class-ların yerinə keçə bilməlidir
- **ISP** (Interface Segregation): client-lər istifadə etmədikləri metodlara bağlı olmamalıdır
- **DIP** (Dependency Inversion): yüksək səviyyəli modullar aşağı səviyyəli modullara deyil, abstraction-lara bağlı olmalıdır

## Praktik Baxış

- SOLID — tool-dur, qayda deyil; kiçik, sadə layihələrdə bəzən over-engineering-ə səbəb ola bilər
- Real qiymətini büyük komandalar, uzun ömürlü layihələr, çox dəyişən tələbatlar zamanı göstərir
- Test yazmaq çətinləşirsə — DIP pozulub; yeni tip əlavə etmək üçün köhnə kodu dəyişirsinizsə — OCP pozulub
- Common mistakes: interface-lər olmadan DIP-dən danışmaq; SRP-ni "bir class = bir metod" kimi anlamaq

### Anti-Pattern Nə Zaman Olur?

**SRP pozuntusu ilə 100 fayllı folder**: hər method üçün ayrı class yaratmaq — `OrderItemPriceCalculator`, `OrderItemTaxApplier`, `OrderItemDiscountSubtractor` — bir sadə hesablama üçün 10 class. Kod parçalanır, iz sürməsi çətin olur. SRP "bir dəyişmə səbəbi" deməkdir, "bir metod" deyil.

**DIP ilə indirection hell**: hər şey üçün interface yaratmaq — `LoggerInterface`, `CacheInterface`, `TranslatorInterface`, sadə `StringFormatter`-ə belə interface — class-lar tanıya bilmək imkansızlaşır. DIP real dəyişim ehtimalı olan yerlər üçündür; heç vaxt dəyişməyəcək implementation-lar üçün interface boş yerdir.
...
1. Mövcud bir Laravel Controller-i götürün (tercihen 150+ sətir); hər SOLID pozuntusunu müəyyən edin; SRP tətbiq edərək Service, Repository, Job-lara bölün
2. E-commerce üçün payment sistemi qurun: `PaymentGatewayInterface`, `StripeGateway`, `PayPalGateway`, `BankGateway`; OCP — yeni gateway mövcud kodu dəyişdirmədən əlavə edilsin
3. `UserRepository`-ni ISP-yə uyğun parçalayın: `CanFindUser`, `CanWriteUser`, `CanSearchUser`, `CanReportUser`; hər service yalnız lazımlı interface-i inject etsin
4. DIP testi: mövcud bir service-in bütün `new ClassName()` çağırışlarını müəyyən edin; interface-lər yazın; Service Container-da bind edin; unit test-lərdə fake inject edin

## Əlaqəli Mövzular

- [Design Patterns Ümumi Baxış](01-design-patterns-overview.md) — SOLID pattern-lərin düzgün tətbiqinin əsasıdır
- [GRASP Prinsipləri](03-grasp-principles.md) — SOLID-i tamamlayan məsuliyyət anlayışları
- [Repository Pattern](../laravel/01-repository-pattern.md) — DIP + ISP-nin real tətbiqi
- [Service Layer](../laravel/02-service-layer.md) — SRP tətbiqinin ən real nümunəsi
- [DI vs Service Locator](../laravel/06-di-vs-service-locator.md) — DIP-in Laravel-dəki implementasiyası
- [Code Smells](../general/02-code-smells-refactoring.md) — SOLID pozuntularının əlamətləri

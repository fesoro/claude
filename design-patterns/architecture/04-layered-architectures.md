...
Layered Architecture — tətbiqin fərqli məsuliyyətlərini ayrı qatlara (layer) böldüyü arxitektura yanaşmasıdır. Ənənəvi variant yuxarıdan aşağıya asılılıq istiqaməti ilə işləyir: Presentation → Business → Persistence → Database. Bu yanaşmanın problemi business layer-in infrastructure-a (DB) birbaşa asılı olmasıdır. Hexagonal, Clean və Onion Architecture-lar bu problemi həll edir: asılılıqlar həmişə içəriyə baxır, domain/business logic framework-dən müstəqildir.

## Niyə Vacibdir

5+ illik developer kimi "bizdə layered architecture var" dedikdə hansı variantı nəzərdə tutduğunuzu, onun trade-off-larını bilməlisiniz. Tipik problem: yeni DB-yə keçmək istədikdə onlarca service dəyişdirilir — bu klasik layered architecture-un dependency direction problemidir. Hexagonal/Onion/Clean bu problemi həll edir, amma boilerplate artır. Seçim mürəkkəbliyin gəlirini ödəyəcəkmi — bunu qiymətləndirib müdafiə edə bilmək lazımdır.

## Əsas Anlayışlar

- **Ənənəvi Layered**: Presentation → Business → Persistence → DB; asılılıq aşağıya; DB dəyişdikdə yuxarı qatlar dəyişir
- **Dependency Rule**: asılılıqlar yalnız içəriyə işarə edir (Hexagonal/Clean/Onion-un əsas qaydası)
- **Domain/Business Core**: framework, DB, xarici API bilmir; pure PHP; test etmək üçün real infrastructure lazım deyil
- **Port**: domain-in kənara ilə kommunikasiya nöqtəsi — interface; nüvədə tərif edilir
- **Adapter**: port-un konkret implementasiyası — kənarda yaşayır; EloquentOrderRepository, SendgridEmailAdapter
- **DIP (Dependency Inversion)**: bu arxitekturaların hamısının əsası; yüksək səviyyəli modul aşağı səviyyəliyə asılı olmaz, hər ikisi interface-ə asılıdır

## Praktik Baxış

- **Real istifadə**: ənənəvi layered — sadə Laravel CRUD-lar, orta mürəkkəblik; Hexagonal/Clean/Onion — mürəkkəb domain, multiple delivery mechanism (HTTP+CLI+Queue), DDD ilə birlikdə
- **Trade-off-lar**: ənənəvi layered — az boilerplate, sürətli inkişaf; Clean/Onion — yüksək test coverage, framework müstəqilliyi, lakin əhəmiyyətli boilerplate, yüksək cognitive overhead
- **Hansı hallarda istifadə etməmək**: Clean/Onion-u kiçik CRUD tətbiqinə tətbiq etmək — over-engineering; startup MVP üçün ənənəvi layered daha praktikdir
- **Common mistakes**: layer-i skip etmək (controller-dən birbaşa DB); business logic-i presentation layer-da yazmaq; domain entity-lərə Eloquent-i extend etdirmək

### Anti-Pattern Nə Zaman Olur?

**Asılılıq istiqaməti olmayan layer-lər**: controller birbaşa repository-ni çağırır — business layer yoxdur. Bir dəyişiklik ripple effect yaradır. "Layered architecture var" deyib layer-lar arası dependency-ni enforce etməmək ən yayılmış anti-pattern-dir. Architecture test-ləri (phparkitect, deptrac) ilə enforce edin.

**Controller birbaşa repository çağırır**: `UserController → UserRepository` — business layer atlanır, business rule-lar controller-ə sızır, test etmək çətindir. Controller yalnız HTTP request/response ilə məşğul olmalı, business logic Service-ə aid olmalıdır.
...
1. Mövcud ənənəvi layered Laravel layihənizin bir modulunu götürün; Hexagonal-a keçid planı hazırlayın: hansi class-lar Port olacaq, hansi Adapter; folder strukturunu kağız üzərində çəkin
2. `phparkitect` və ya `deptrac` konfiqurasiya edin; Controller-dən birbaşa Repository çağırışını qadağan edən rule yazın; layihənizdə neçə pozuntu var — tapın
3. `OrderService`-in `PlaceOrder` use case-ini götürün; ənənəvi layered, Hexagonal, Onion — üç implementasiya yazın; test coverage müqayisə edin; boilerplate fərqini ölçün
4. Domain entity-lərdən `extends Model`-i çıxarın; Mapper pattern yazın (`OrderMapper::toDomain()`, `OrderMapper::toModel()`); Infrastructure qatında saxlayın; Domain unit test-lərini framework olmadan çalışdırın

## Əlaqəli Mövzular

- [Hexagonal Architecture](05-hexagonal-architecture.md) — Ports & Adapters — bu faylda müqayisəli izah
- [Onion Architecture](06-onion-architecture.md) — DDD ilə uyğun variant
- [SOLID Prinsipləri](02-solid-principles.md) — DIP bu arxitekturaların əsasıdır
- [Repository Pattern](../laravel/01-repository-pattern.md) — secondary port/adapter-ın ən real nümunəsi
- [DDD](../ddd/01-ddd.md) — Clean/Onion ilə DDD birlikdə istifadə olunur
- [CQRS](../integration/01-cqrs.md) — Application layer CQRS ilə tamamlanır

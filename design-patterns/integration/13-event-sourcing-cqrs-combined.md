...
- [CQRS](01-cqrs.md) — command/query ayrılması; bu faylın write tərəfinin əsasıdır
- [Event Sourcing](02-event-sourcing.md) — event store, domain event, reconstitution
- [CQRS Read Model](14-cqrs-read-model-projection.md) — projection-ların dərin izahı
- [Event Sourcing Snapshots](15-event-sourcing-snapshots.md) — böyük event axını üçün performance
- [Outbox Pattern](04-outbox-pattern.md) — event-lərin reliable publish edilməsi
- [DDD Aggregates](../ddd/04-ddd-aggregates.md) — event-sourced aggregate DDD aggregate-dir
- [Domain Events](../ddd/05-ddd-domain-events.md) — event-lərin domain semantikası

---

## Anti-Pattern Nə Zaman Olur?

**İkisini birlikdə sadə app-a tətbiq etmək:**
Event Sourcing + CQRS birlikdə 5+ class, event store, projection, snapshot, read model — sadə blog tətbiqi üçün bu complexity-i justify etmək olmur. Hər ikisi ayrıca da mürəkkəbdir; birlikdə learning curve çox dik, komanda hazır olmalıdır. Real audit trail, temporal query, ya da complex domain olmadan bu kombinasiya overkill-dir.

**ES olmadan CQRS-i "sadəcə read/write ayrımı" kimi tətbiq etmək:**
Write model oxuyur, read model ayrı cədvəldir, amma event-siz sync edilir — projection rebuild mümkün deyil, data uyuşmazlığı olduqda mənbəyə qayıtmaq olmur. CQRS + ES birlikdə işləməlidir; event-lər həm write-ın audit trail-i, həm projection-ların mənbəyidir.

**Read model üçün event store-u birbaşa sorğulamaq:**
Event store write-optimized-dır. `SELECT * FROM event_store WHERE aggregate_id=? ORDER BY version` hər read-də — mürəkkəb sorğular üçün deyil. Read model-lər (projection) yaradın: denormalized, sorğuya uyğun cədvəllər; event store yalnız write üçün.

## Praktik Tapşırıqlar

1. `Order` aggregate yazın: `OrderCreated`, `ItemAdded`, `OrderConfirmed`, `OrderShipped` event-ləri; `reconstitute()` metodu — event-lərdən state rebuild; test: 5 event sonra düzgün state
2. `MySQLEventStore` yazın: `append()` optimistic concurrency ilə; `load()` + `loadFromVersion()`; projection: `OrderListProjector` `order_summary_view`-u yeniləsin
3. CQRS + ES tam integration: `PlaceOrderCommand` → `CreateOrderHandler` → ES append; `GetOrderQuery` → `OrderQueryService` → read model; test: write sonra read, eventual consistency lag simulyasiyası
4. Projection rebuild command yazın: `projections:rebuild:orders`; event store-dan başdan replay; shadow rebuild — köhnə cədvəl dolu qalır, yeni dolduqdan sonra atomik swap

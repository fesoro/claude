...
- [CQRS](01-cqrs.md) — read model CQRS-in query tərəfinin implementasiyasıdır
- [Event Sourcing + CQRS Combined](13-event-sourcing-cqrs-combined.md) — ES ilə projection rebuild
- [Event Sourcing](02-event-sourcing.md) — event-lərdən projection yaratmaq
- [Outbox Pattern](04-outbox-pattern.md) — projection-ları trigger edən event-lərin reliable delivery-si
- [Cache-Aside](../general/07-cache-aside.md) — read model-in üstündə caching qatı

---

## Anti-Pattern Nə Zaman Olur?

**Read model-in çox granular olması:**
Hər entity üçün ayrı read model cədvəli — `order_header_view`, `order_items_view`, `order_customer_view` — amma UI hər sorğuda hər üçünü tələb edir. 3 query = sequential read, join-siz demək read model-in faydasını itirir. Use-case-ə uyğun denormalized cədvəl: `order_detail_view` hər şeyi bir sırada saxlasın.

**Eventual consistency-nin UI-da görünsə:**
User sifariş verir, dərhal sifariş listini yeniləyir — async projection işlənməyib, yeni sifariş görünmür. Bu user-ə bug kimi görünür. Həll: yazıdan dərhal sonra gələn oxuma write DB-dən edilsin; ya da optimistic UI — client local state-i göstərsin, arxa planda refresh etsin. Eventual consistency qəbul edilsə UI-a bildirmək lazımdır.

**Async projection-ı payment confirmation üçün istifadə etmək:**
Ödəniş tamamlandıqdan sonra balance sorğusu async projection-dan gəlir — lag olduğu üçün köhnə balans göstərilir. Kritik, real-time data (ödəniş statusu, balans) həmişə write DB-dən oxusun; async projection yalnız non-critical display üçün.

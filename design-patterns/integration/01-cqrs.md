...
**Sadə CRUD üçün CQRS:**
`UserController.update()` — adı dəyişdirmək üçün `UpdateUserNameCommand`, `UpdateUserNameHandler`, `UserUpdatedEvent`, ayrı read model — 5 class, 1 field update üçün. CQRS complexity artırır, benefit vermir. Yalnız real write/read asimmetriyası olduqda istifadə et.

**Real ayrılma olmadan ad dəyişikliyi:**
Command/Query class-ları var, amma handler-lər eyni Eloquent model-i həm write, həm read üçün istifadə edir. Bu yalnız abstraction layer-dır, real CQRS deyil. Read model-i denormalize etmək üçün əlavə iş görülmürsə, CQRS-in faydası yoxdur.

**Command handler state qaytarır:**
```php
// YANLIŞ — command handler query kimi davranır
class CreateOrderHandler {
    public function handle(CreateOrderCommand $command): Order  // domain object qaytarır!
    {
        $order = Order::create([...]);
        return $order;  // Command side query işi görür
    }
}

// DÜZGÜN — yalnız ID qaytarır, sonra query ilə data al
class CreateOrderHandler {
    public function handle(CreateOrderCommand $command): string  // yalnız ID
    {
        $order = Order::create([...]);
        return $order->id;
    }
...
**Read model-i inconsistent saxlamaq:**
Command execute olunur, lakin read model lazımi zamanda güncəllənmir. User `POST /orders` edir, dərhal `GET /orders` edir, yeni sifariş yoxdur — müştəri bug hesab edir. Eventual consistency qəbul edilə bilər, amma UI-a bildirilib, ya da yazıqda inline güncəlləmə edilib.
...
## Əlaqəli Mövzular
- [Event Sourcing](02-event-sourcing.md) — CQRS ilə natural pair; command events generate edir, query projection-dan oxuyur
- [CQRS Read Model](14-cqrs-read-model-projection.md) — read model projection-un dərin izahı
- [Event Sourcing + CQRS Combined](13-event-sourcing-cqrs-combined.md) — ikisi birlikdə real implementasiya
- [Command/Query Bus](../laravel/08-command-query-bus.md) — CommandBus mediator-dır; command-ı handler-ə yönləndirir
- [Repository Pattern](../laravel/01-repository-pattern.md) — command handler repository-dən oxuyur
- [DDD Aggregates](../ddd/04-ddd-aggregates.md) — CQRS DDD aggregate-ləri ilə birlikdə tez-tez işlənir

...
- **Anti-Pattern Nə Zaman Olur?**: Handler-lar bir-birindən birbaşa asılı olduqda — `InventoryHandler` daxilindən `new CreditHandler()` yaratmaq zənciri hardcode edir, dinamikliyi məhv edir; chain qurmaq həmişə xaricdən (Invoker/ServiceProvider) aparılmalıdır. Daha təhlükəlisi: zəncirin sonunda heç bir handler request-i işlətmədikdə susqun keçmək — caller xəta da almır, uğur da; mütləq ya fallback handler, ya da explicit exception olmalıdır. "Silent swallow" production-da debug edilməsi çox çətin bug-lara yol açır.
...
## Əlaqəli Mövzular
- [../structural/03-decorator.md](../structural/03-decorator.md) — Decorator da zəncir qurur amma hamısı işlədilir; CoR isə zənciri dayandıra bilir
- [../laravel/02-service-layer.md](../laravel/02-service-layer.md) — Service-lərdə pipeline logic
- [../laravel/05-event-listener.md](../laravel/05-event-listener.md) — Event listener-lər paralel CoR kimi işləyir
- [07-state.md](07-state.md) — State dəyişikliyi CoR validation ilə birgə işlədilir
- [../laravel/03-pipeline.md](../laravel/03-pipeline.md) — Laravel Pipeline CoR-un framework-dəki implementasiyası

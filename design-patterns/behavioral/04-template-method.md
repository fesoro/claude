...
- **Anti-Pattern Nə Zaman Olur?**: Skeleton çox "qatı" (rigid) olduqda — base class 10 abstract metod tələb edirsə, subclass-lar həmin addımların yarısını `return null` ya da boş array ilə keçiştirməyə başlayır; bu Liskov Substitution Principle pozuntusudur. Inheritance depth artdıqca — `BaseImporter → CsvImporter → SalesCsvImporter → MonthlySalesCsvImporter` — hər override-da hansı loqikanın harada olduğunu izləmək çətinləşir. Bu halda Strategy + Composition daha yaxşı seçimdir: algoritmin dəyişən hissələrini inject olunan object-lərə köçürün.
...
## Əlaqəli Mövzular
- [02-strategy.md](02-strategy.md) — Template Method vs Strategy seçimi; Strategy composition ilə eyni problemi həll edir
- [../creational/03-abstract-factory.md](../creational/03-abstract-factory.md) — Abstract class istifadəsi baxımından oxşarlıq
- [../laravel/02-service-layer.md](../laravel/02-service-layer.md) — Service class-larında pipeline qurmaq
- [../laravel/03-pipeline.md](../laravel/03-pipeline.md) — Pipeline pattern Template Method-un composition ilə alternativi

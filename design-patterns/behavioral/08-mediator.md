...
- **Anti-Pattern Nə Zaman Olur?**: Mediator God Object-ə çevrildikdə — bütün business flow-lar `ConcreteOrderMediator`-da cəmləşir; service-lər mediator-ı inject edir, mediator da service-ləri inject edir; zamanla mediator-ın özü 500+ sətir olur. Bu halda mediator yalnız orchestration edir, business qaydaları isə service-lərdə qalır. Praktiki qayda: mediator-dakı hər `if` bloğu qırmızı xal — koordinasiya "kim → kimi çağırır" sualına cavab verir, "necə işlənir" sualına yox. Laravel-də bu problemi Event-lərlə həll etmək çox vaxt daha skalabel seçimdir.
...
## Əlaqəli Mövzular
- [01-observer.md](01-observer.md) — fərq: Observer event subscription (publisher bilmir), Mediator centralized coordinator (iş axınını bilir)
- [03-command.md](03-command.md) — Command Bus mediator + command pattern birləşməsidir
- [../structural/01-facade.md](../structural/01-facade.md) — Facade da birdən çox subsystem-i koordinasiya edir, lakin unidirectional-dır
- [../laravel/03-pipeline.md](../laravel/03-pipeline.md) — ardıcıl steps üçün mediator alternativi
- [../laravel/08-command-query-bus.md](../laravel/08-command-query-bus.md) — Command Bus mediator pattern-in Laravel implementasiyası

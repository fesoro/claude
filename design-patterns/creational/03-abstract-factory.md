...
**1. "Ailəsi" olmayan Factory — sadəcə Factory Method-u mürəkkəbləşdirir:**
```php
// Əgər hər factory yalnız bir növ obyekt yaradırsа, Abstract Factory lazım deyil
interface LoggerFactoryInterface
{
    public function createLogger(): LoggerInterface;
    // Başqa metod yoxdur — bu sadəcə Factory Method-dur, Abstract Factory deyil
}
// Həll: birbaşa Factory Method və ya DI konteyner istifadə et
```

**2. Uyğunsuz (inconsistent) family — Abstract Factory-nin əsas vədi pozulur:**
```php
// Problem: EmailFactory SMS sender-i qaytarırsa, family concept pozulur
class EmailNotificationFactory implements NotificationFactoryInterface
{
    public function createSender(): SenderInterface
    {
        return new SmsSender(); // YANLIŞ — email factory sms sender qaytarır
    }
}
// Abstract Factory-nin mənası ailəni consistent saxlamaqdır
```

**3. Hər configuration fərqi üçün yeni Concrete Factory:**
```php
// 10 tenant-ın hər birisi üçün ayrı factory class yazmaq yanlışdır
class Tenant1Factory implements NotificationFactoryInterface { ... }
class Tenant2Factory implements NotificationFactoryInterface { ... }
// ... 10 class, eyni kod, fərqli config dəyərləri
// Həll: bir factory, konstruktorda config qəbul edir
class ConfigurableNotificationFactory implements NotificationFactoryInterface
{
    public function __construct(private readonly array $config) {}
    // config-ə görə doğru implement-i qaytarır
}
```

**4. Interface-ə metod əlavə etmək — bütün implementation-ları qırır:** Abstract Factory interface-inə `createTracker()` əlavə etmək bütün Concrete Factory-ləri (test factory-si daxil) dəyişdirir. Əgər bu çox baş verirsə, interface stabilliyini yenidən düşünmək lazımdır.
...
## Əlaqəli Mövzular
- [Factory Method](02-factory-method.md) — Abstract Factory, bir neçə Factory Method-un ailəsidir
- [Builder](04-builder.md) — Builder mürəkkəb tək obyekt yaradır, Abstract Factory bir-biri ilə uyğun obyektlər ailəsi
- [Strategy](../behavioral/02-strategy.md) — Concrete Factory-lər runtime-da Strategy kimi dəyişdirilə bilər
- [Service Layer](../laravel/02-service-layer.md) — Abstract Factory vasitəsilə yaranan service-lər Service Layer-ə inject edilir

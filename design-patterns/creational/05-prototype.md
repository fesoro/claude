...
**1. Sadə obyektlər üçün clone — `new`-dan daha çətin:**
```php
// Pis: sadə value object-i clone etmək
$money = clone $originalMoney; // __clone() ilə mürəkkəblik yaradırsan
$money->amount = 500;

// Yaxşı: sadəcə yeni yarart
$money = new Money(500, 'AZN'); // aydın, ucuz, immutable
```

**2. Shallow copy aldatması — gizli shared state:**
```php
class Config
{
    public array $settings = [];
    public \Redis $redis; // resource — clone-da eyni connection!
}

$copy = clone $config;
$copy->redis->set('key', 'value'); // original-ın Redis connection-una yazır!
// Problem: __clone() yazılmadığı üçün resource paylaşılır
```

**3. `replicate()` ilə relation-ları da kopyalamağı unutmaq:**
```php
// Laravel replicate() yalnız attributes-u kopyalayır, relation-ları kopyalamır
$copy = $product->replicate();
$copy->save();
// product-ın tags, categories, images relation-ları kopyalanmadı!

// Düzgün yol: relation-ları da ayrıca əlavə et
$copy = $product->replicate();
$copy->save();
$copy->tags()->sync($product->tags->pluck('id')); // explicit sync
```

**4. Prototype Registry-ni Cache kimi istifadə etmək:** Registry template-lər saxlayır, clone edir. Əgər template-lər heç vaxt dəyişmirsə, bu sadəcə object factory-dir — registry gereksiz overhead-dır. Registry yalnız template-lərin runtime-da dəyişə biləcəyi hallarda dəyər verir.
...
## Əlaqəli Mövzular
- [Builder](04-builder.md) — Builder sıfırdan qurur, Prototype mövcudu kopyalayır
- [Factory Method](02-factory-method.md) — Factory Method yeni obyekt yaradır, Prototype mövcudu klonlayır
- [Object Pool](06-object-pool.md) — Pool-da hazır saxlanılan obyektlər Prototype ideyasını tamamlayır
- [State](../behavioral/07-state.md) — Prototype Registry-də template state-lərini idarə etmək üçün

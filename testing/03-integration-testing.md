...
- **Unit test-in tapa bilmədiyi bug-lar**: Repository unit test keçsə də, real database-ə yazarkən foreign key constraint, unique index xətaları yalnız integration test ilə tutulur
- **Service layer doğruluğu**: `OrderService` + `InventoryService` + `PaymentService` arasındakı əlaqə unit testdə mock olunur — yalnız integration test real davranışı yoxlayır
- **Database query düzgünlüyü**: Eloquent scope-lar, join-lər, aggregation query-ləri real database-də sınaqdan keçirilir, çünki mock onları əvəz edə bilmir
- **Cache invalidation**: Cache layer ilə database arasındakı sinxronizasiya yalnız integration test ilə düzgün sınaqdan keçirilir
- **Transaction bütövlüyü**: Rollback məntiqinin düzgün işlədiyini yalnız real database transaction-ı ilə yoxlamaq mümkündür
